...
// This method is most generated by Claude AI
...
func loadPrograms(filename string) (map[string]*Program, error) {
	...
	firstLine := true
	// This for loop is most generated by Claude AI
	for scanner.Scan() {
		line := strings.TrimSpace(scanner.Text())
		if firstLine {
			firstLine = false
...
// This method is most generated by Claude AI
func printResults(w io.Writer, residents map[int]*Resident, programs map[string]*Program) {
	...
	fmt.Fprintln(w, "lastname,firstname,residentID,programID,name")

	unmatchedCount := 0
	for _, r := range list {
		if r.matchedProgram == "" {
			// Unmatched resident uses XXX and NOT_MATCHED as per spec
			fmt.Fprintf(w, "%s,%s,%d,XXX,NOT_MATCHED\n",
	...
	fmt.Fprintf(w, "Number of unmatched residents: %d\n", unmatchedCount)
	fmt.Fprintf(w, "Number of positions available: %d\n", openPositions)
}

// Main

// renew main function by Claude AI, and marge the output method. To make the output file
// at the same folder of this file.
func main() {
	if len(os.Args) != 3 {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "Error creating output.txt:", err)
		os.Exit(1)
	}
	defer outFile.Close()

	// MultiWriter: every write goes to both terminal and output.txt at once
	...
	// Write results to both terminal and file
	printResults(w, residents, programs)
	fmt.Fprintf(w, "\nExecution time: %s\n", end.Sub(start))
}
